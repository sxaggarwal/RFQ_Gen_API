...
class MieTrak:
    ...
    def insert_part_details_in_item(self, item_pk, part_number, values, item_type = None):
        if item_type == 'Material':
            ...
            self.item_table.update(item_pk, StockLength=values[0], Thickness=values[1], StockWidth=values[2], Weight=values[3], PartLength=values[0], PartWidth=values[2], PurchaseOrderComment=po_comment, ManufacturedItem=0, Purchase=1, ShipLoose=0, BulkShip=0)
        else:
