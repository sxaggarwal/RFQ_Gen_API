...
class MieTrak:
    ...
    def __init__(self):
        ...
        self.quote_assembly_formula_variable_table = TableManger(
    ...
    def get_customer_data(self, selected_customer_index=None, names=False):
        ...
        if names is True:
            ...
            selected_customer_data = self.party_table.get(
                "ShortName", "Email", PartyPK=party_pk
            )
    ...
    def get_buyer_info(self, buyer_fk):
        ...
        selected_customer_data = self.party_table.get(
            "ShortName", "Email", PartyPK=buyer_fk
        )
        if selected_customer_data:
            short_name, email = selected_customer_data[0]
        else:
    ...
    def get_address(self, party_fk):
        ...
        billing_details = self.address_table.get(
            "AddressPK",
            "Name",
            "Address1",
            "Address2",
    ...
    def get_or_create_item(
        ...
        vendor_unit=1.0,
        manufactured_item=0,
        calculation_type_fk=1,
        inventoriable=1,
        purchase_order_comment=None,
        description=None,
        comment=None,
        only_create=None,
        bulk_ship=1,
        ship_loose=1,
        cert_reqd_by_supplier=0,
        can_not_create_work_order=0,
        can_not_invoice=0,
        general_ledger_account_fk=100,
        purchase_account_fk=116,
        cogs_acc_fk=116,
    ):
        ...
        if only_create is None:
            ...
            if item_pk:
                ...
                item_inventory_pk = self.item_inventory_table.insert(
                    inventory_info_dict
                )
                item_info_dict = {
    ...
    def create_bom_quote(
        ...
        part_width=0.00,
        part_length=0.000,
        parts_required=1.000,
        quantity_reqd=1.000,
        min_piece_price=0.00,
        parts_per_blank_scrap_percentage=0.000,
        markup_percentage_1=9.999999,
        piece_weight=0.000,
        custom_piece_weight=0.0000,
        piece_cost=0.0000,
        piece_price=0.00000,
        stock_pieces=0,
        stock_pieces_scrap_perc=0.000,
        calculation_type_fk=17,
        unattended_operation=0,
        do_not_use_delivery_schedule=0,
        vendor_unit=1.00000,
        grain_direction=0,
        parts_per_blank=1.000,
        against_grain=0,
        double_sided=0,
        cert_reqd=0,
        non_amortized_item=0,
        pull=0,
        not_include_in_piece_price=0,
        lock=0,
        nestable=0,
        bulk_ship=0,
        ship_loose=0,
        customer_supplied_material=0,
        thickness=0.00,
    ):
    ...
    def insert_part_details_in_item_new(
        self, item_pk, part_number, values, item_type=None
    ):
        """Similar to the other insert_part_details function just the structure of value list is different"""
        if item_type == "Material":
            po_comment = (
                f" Dimensions (L x W x T): {values[14]} x {values[15]} x {values[16]}"
            )
            self.item_table.update(
                item_pk,
                StockLength=values[14],
                Thickness=values[16],
                StockWidth=values[15],
                Weight=values[4],
                PartLength=values[1],
                PartWidth=values[3],
                PurchaseOrderComment=po_comment,
            ...
            self.item_table.update(
                ...
                StockWidth=values[15],
                Weight=values[4],
                DrawingNumber=values[8],
                DrawingRevision=values[9],
                Revision=values[11],
                PartLength=values[1],
                PartWidth=values[3],
                VendorPartNumber=part_number,
    ...
    def create_quote_assembly_formula_variable(self, quote_pk):
        ...
        temp = self.quote_assembly_table.get(
            "QuoteAssemblyPK",
            "SetupFormulaFK",
            "RunFormulaFK",
            "OperationFK",
    ...
    def delete_rfq_line_pk(self, rfq_pk):
        ...
        rfq_line_pk = self.rfq_line_table.get(
            "RequestForQuoteLinePK", "QuoteFK", RequestForQuoteFK=rfq_pk
        )
        if rfq_line_pk:
