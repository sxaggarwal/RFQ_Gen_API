...
def pk_info_dict(info_dict):
    ...
    for key, value in info_dict.items():
        ...
        ht_pk = None
        if value[5]:
            result = item_table.get(
                "ItemPK",
                PartNumber=value[5],
                StockLength=value[14],
                StockWidth=value[15],
                Thickness=value[16],
            )
            if result:
                ...
                pk = m.get_or_create_item(
                    ...
                    purchase_account_fk=127,
                    cogs_acc_fk=127,
                    calculation_type_fk=4,
                )
                mat_pk = pk
        ...
        if value[6]:
            ...
            if result:
                ...
                pk = m.get_or_create_item(
                    ...
                    item_type_fk=5,
                    comment=value[6],
                    purchase_order_comment=value[6],
                    inventoriable=0,
                    only_create=1,
                    cert_reqd_by_supplier=1,
        ...
        if value[7]:
            ...
            if result:
                ...
                pk = m.get_or_create_item(
                    ...
                    description=value[7],
                    comment=value[7],
                    purchase_order_comment=value[7],
                    inventoriable=0,
                    only_create=1,
                    cert_reqd_by_supplier=1,
                    can_not_create_work_order=1,
